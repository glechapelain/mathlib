...
Matrix3 Matrix3::invert()
{
	...
	float invDeterminant = 1.0f / determinant;
	float3D row0 = invDeterminant * float3D{ tmp[0], -tmp[3], tmp[6] };
	float3D row1 = invDeterminant * float3D{ -tmp[1], tmp[4], -tmp[7] };
	float3D row2 = invDeterminant * float3D{ tmp[2], -tmp[5], tmp[8] };
